`timescale 1ns / 1ps
// Top-level module
module ReconfigMM2(input wire clk, input wire rst, input wire enable, output wire done);
  ...
  RAMDataB #(.DATA_WIDTH(DATA_WIDTH), .ADDR_WIDTH(ADDR_WIDTH)) 
  bramMatB (
    .address(bramAddrB),
    .clock(clk),  // Assuming there's a clock signal
    .write_enable(1'b0),  // Set to 1 for writing, 0 for reading
  ...
  MatMult RMM (.Clock(clk), .reset(rst), .Enable(enable), .A(inputBramDataA), .B(inputBramDataB), .C(RMMOutput), .done(done));
  ...
  //end
  initial begin
    f = $fopen("C:\Users\ojasa\OneDrive\Documents\Research\REU2023\ReconfigMM2\matOut.txt","w");
